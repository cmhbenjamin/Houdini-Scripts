...
def default(setting):
    ...
    setting.antialias(flipbookAntialias.HighQuality)
...
def settingsToList(settings):
    ...
    for m in insp:
        name = m[0]
        if not (name.startswith('__') or name not in _ignore):
            try:
...
def load():
    ...
    if(not settingNode):
        obj = hou.node("/obj")
        obj.createNode("geo", _settingNode, run_init_scripts=False)
        default(settings)
        setting_node.setUserData("flipbooksetting", listToJson(settingToList(settings),0))
    else:
        listToSetting(settings,jsonToList(setting_node.userData("flipbooksetting")))
"""
    initialize setting Node
"""
def save():
